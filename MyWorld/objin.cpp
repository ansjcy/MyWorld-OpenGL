...
glmFirstPass(GLMmodel* model, FILE* file)
{
	...
	while (fscanf(file, "%s", buf) != EOF) {
		switch (buf[0]) {
			...
			case 'g':               /* group */
				...
				//cout << "Group " << group->name << " added" << endl;
				break;
			case 'f':               /* face */
				v = n = t = 0;
				fscanf(file, "%s", buf);
	...
	group = model->groups;
	while (group) {
		//cout << "Creating space for group " << group->name << endl;
		//cout << "Total " << group->numtriangles << " triangles" << endl;
		//system("pause");
	...
	while (fscanf(file, "%s", buf) != EOF) {
		switch (buf[0]) {
			...
			case 'u':
				fgets(buf, sizeof(buf), file);
				sscanf(buf, "%s %s", buf, buf);
				//cout << "Finding material: " << buf << endl;
			...
			case 'f':               /* face */
				...
				if (strstr(buf, "//")) {
					...
					T(numtriangles).vindices[0] = v < 0 ? v + numvertices : v;
					T(numtriangles).tindices[0] = t < 0 ? t + numtexcoords : t;
					T(numtriangles).nindices[0] = 0.0;
					fscanf(file, "%d/%d", &v, &t);
					T(numtriangles).vindices[1] = v < 0 ? v + numvertices : v;
					T(numtriangles).tindices[1] = t < 0 ? t + numtexcoords : t;
					T(numtriangles).nindices[1] = 0.0;
					fscanf(file, "%d/%d", &v, &t);
					T(numtriangles).vindices[2] = v < 0 ? v + numvertices : v;
					T(numtriangles).tindices[2] = t < 0 ? t + numtexcoords : t;
					T(numtriangles).nindices[2] = 0.0;
					group->triangles[group->numtriangles++] = numtriangles;
					numtriangles++;
					while (fscanf(file, "%d/%d", &v, &t) > 0) {
						T(numtriangles).vindices[0] = T(numtriangles - 1).vindices[0];
...
glmReadOBJ(char* filename)
{
	...
	//cout << "Number of texcoords: " << model->numtexcoords << endl;
	...
	while (group)
	{
		//cout << "In group " << group->name << endl;
		for (i = 0; i < group->numtriangles; i++)
		{
			//cout << "Triangle " << group->triangles[i] << " found" << endl;
		}
		group = group->next;
		//system("pause");
	}
...
glmDraw(GLMmodel* model, GLuint mode)
{
	...
	while (group) {
		...
		for (i = 0; i < group->numtriangles; i++) {
			...
			if (mode & GLM_TEXTURE)
			{
				//cout << group->name <<" " << i << "/" << group->numtriangles << " : Using texcoord[" << 1 * triangle->tindices[0] << "]" << endl;
...
GLvoid
glmDraw(GLMmodel* model, GLMtexture *texture, GLuint mode)
{
	static GLuint i;
	static GLMgroup* group;
	static GLMtriangle* triangle;
	...
	if (mode & GLM_MATERIAL && (!model->materials || texture==NULL)) {
		printf("glmDraw() warning: material render mode requested "
					 "with no materials defined.\n");
		mode &= ~GLM_MATERIAL;
	}
...
bool LoadTGA(GLMtexture *texture, char *filename)         // Loads A TGA File Into Memory  
{      
