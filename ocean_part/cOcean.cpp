...
void cOcean::driftOcean(float deltaX, float deltaZ)
{
	dx = deltaX;
	dz = deltaZ;
}
...
void cOcean::render(float t, glm::vec3 light_pos, glm::mat4 Projection, glm::mat4 View, glm::mat4 Model, bool use_fft,bool iswire) {
...
//	Model = glm::translate(Model, glm::vec3(dx, 0, dz));
//	glDisable(GL_BLEND);
	for (int j = -10; j < 11; j++) {
		for (int i = -10; i <11; i++) {
			...
			glUniformMatrix4fv(model, 1, GL_FALSE, glm::value_ptr(Model));
			glDrawElements(iswire ? GL_LINES : GL_TRIANGLES, indices_count, GL_UNSIGNED_INT, 0);
		}
	}	
	//glEnable(GL_BLEND);
